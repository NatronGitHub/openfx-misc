...
#ifdef USE_OSMESA
struct TestWarpPlugin::OSMesaPrivate
{
    OSMesaPrivate(TestWarpPlugin *effect)
        : _effect(effect)
        , _ctx(0)
        , _ctxFormat(0)
        , _ctxDepthBits(0)
        , _ctxStencilBits(0)
        , _ctxAccumBits(0)
        , _ctxCpuDriver(TestWarpPlugin::eCPUDriverSoftPipe)
        , _openGLContextData()
    {
    }
    ...
    GLint _ctxAccumBits;
    TestWarpPlugin::CPUDriverEnum _ctxCpuDriver;
    OpenGLContextData _openGLContextData;
};

void
TestWarpPlugin::initMesa()
{
}

void
TestWarpPlugin::exitMesa()
{
...
void
TestWarpPlugin::RENDERFUNC(const OFX::RenderArguments &args)
{
    ...
    if (_from) {
        _from->getValueAtTime(time, fromx, fromy);
    }
    if (_to) {
        _to->getValueAtTime(time, tox, toy);
    }
    if (_direction) {
        backward = (_direction->getValueAtTime(time) == 1);
    }
    if (_mipmap) {
...
# endif
    ...
    if (mipmap && !supportsMipmap) {
        DPRINT( ("TestWarp: legacy mipmap generation!\n") );
    ...
    rndGLStateLogFunction("TestWarpRender");
#else
    ...
    /////////////////////////////////////////////////////////////
    // prepare the VBO and IBO to draw the triangle strip mesh
    ...
    // http://www.learnopengles.com/android-lesson-eight-an-introduction-to-index-buffer-objects-ibos/#comment-1473
    // http://www.learnopengles.com/android-lesson-eight-an-introduction-to-index-buffer-objects-ibos/#comment-1563
    /////////////////////////////////////////////////////////////
    ...
    if (anisotropic) {
        if (haveAniso) {
            ...
            DPRINT( ("TestWarp: anisotropic texture filtering not available.\n") );
        }
    }
 ...
 */
void*
TestWarpPlugin::contextAttached(bool createContextData)
{
    ...
    return NULL;
} // TestWarpPlugin::contextAttached
 ...
 */
void
TestWarpPlugin::contextDetached(void* contextData)
{
#ifdef USE_OPENGL
    if (contextData) {
...
#ifdef USE_OSMESA
bool
TestWarpPlugin::OSMesaDriverSelectable()
{
