...
{
    CompBuf *cbuf= (CompBuf *)MEM_callocN(sizeof(CompBuf), "compbuf");
    ...
    if (alloc) {
        switch (cbuf->type) {
            case CB_RGBA:
                cbuf->rect = (float*)MEM_callocN(4*sizeof(float)*sizex*sizey, "compbuf RGBA rect");
                break;

            case CB_VEC3:
                cbuf->rect = (float*)MEM_callocN(3*sizeof(float)*sizex*sizey, "compbuf Vector3 rect");
                break;

            case CB_VEC2:
                cbuf->rect = (float*)MEM_callocN(2*sizeof(float)*sizex*sizey, "compbuf Vector2 rect");
                break;

            case CB_VAL:
                cbuf->rect = (float*)MEM_callocN(sizeof(float)*sizex*sizey, "compbuf Fac rect");
                break;
        }
...
static float getWeight(BokehCoeffs* BKH, int len_bkh, float u, float v, float rad, float inradsq)
{
    ...
    while (len_bkh--) {
        if ((bc->dy*(u - bc->x0) - bc->dx*(v - bc->y0)) > 0.f) {
            return 0.f;
        }
...
#define YVV(L)                                                          \
    ...
    sz = MAX2(buf->x, buf->y);
    Y = (float*)MEM_callocN(sz*sizeof(float), "IIR_gauss Y buf");
    W = (float*)MEM_callocN(sz*sizeof(float), "IIR_gauss W buf");
    // H
    for (y=0; y<buf->y; y++) {
        X = &buf->rect[y*buf->x];
        YVV(buf->x);
    ...
    // V
    X = (float*)MEM_callocN(buf->y*sizeof(float), "IIR_gauss X buf");
    for (x=0; x<buf->x; x++) {
        for (y=0; y<buf->y; y++) {
                             ...
                             float inpval, int no_zbuf, CameraInfo *cinfo)
{
    ...
    if (cinfo) {
        cam_lens = cinfo->lens;
        cam_fdist = cinfo->fdist;

        if (cam_fdist==0.0)
            cam_fdist = 1e10f; /* if the dof is 0.0 then set it be be far away */

        cam_invfdist = 1.0f / cam_fdist;
    }
    ...
    if (no_zbuf) {
        ...
        for (p=0; p<(unsigned int)(img->x*img->y); p++) {
            ...
            if (nqd->maxblur != 0.f) {
                crad->rect[p] = MIN2(crad->rect[p], nqd->maxblur);
            }
        }
    } else {
        float wt;
        ...
        for (y=0; y<img->y; y++) {
            ...
            for (x=0; x<img->x; x++) {
                ...
                if (crad->rect[px] <= 0.f) {
                    ...
                    if (crad->rect[px] > maxfgc) {
                        maxfgc = crad->rect[px];
                    }
                } else {
                    crad->rect[px] = wts->rect[px] = 0;
                }
            }
        }
        ...
        for (y=0; y<img->y; y++) {
            ...
            for (x=0; x<img->x; x++) {
                ...
                if (zbuf->rect[px]!=0.f) {
                    ...
                    // if maxblur!=0, limit maximum
                    if (nqd->maxblur != 0.f) {
                        crad->rect[px] = MIN2(crad->rect[px], nqd->maxblur);
                    }
                } else {
                    crad->rect[px] = 0.f;
                }
                  ...
                  float inpval, int no_zbuf, CameraInfo *cinfo)
{
    ...
    float bkh_b[4] = {0};    // shape 2D bound
    float cam_fdist=1, cam_invfdist=1, cam_lens=35;
    float dof_sp, bk_hn_theta=0, inradsq=0;
    ...
    for (y = y0; y < y1; y++) {
        ...
        for (x=0; x<img->x; x++) {
            ...
            if (ct_crad==0.f) {
                ...
                result->rect[cp4] = img->rect[cp4];
                if (result->type != CB_VAL) {
                    result->rect[cp4+1] = img->rect[cp4+1];
                    result->rect[cp4+2] = img->rect[cp4+2];
                    result->rect[cp4+3] = img->rect[cp4+3];
                }
                continue;
            }
            cR2 *= cR2;
...
#define TESTBG4(c, w) {                                         \
                    if (ct_crad > nqd->bthresh) {               \
                        if (crad->rect[p] > nqd->bthresh) {     \
                            result->rect[p4] += c[0];           \
                            result->rect[p4+1] += c[1];         \
                            result->rect[p4+2] += c[2];         \
                            result->rect[p4+3] += c[3];         \
                            wts->rect[p] += w;                  \
                        }                                       \
                    } else {                                    \
                        result->rect[p4] += c[0];               \
...
#define AAPIX(a, b) {                                                   \
                        int _ny = b;                                    \
                        if ((_ny >= 0) && (_ny < result->y)) {          \
                            int _nx = a;                                \
                            if ((_nx >=0) && (_nx < result->x)) {       \
                                p = _ny*result->x + _nx;                \
                                if (result->type==CB_VAL) {             \
                                    TESTBG1(aacol, lwt);                \
                                }                                       \
...
#define CSCAN(a, b) {                                                   \
                        ...
                        if ((_ny >= 0) && (_ny < result->y)) {          \
                            ...
                            if (xs < 0) xs = 0;                         \
                            xe = x + a;                                 \
                            if (xe > result->x) xe = result->x;         \
                            p = _ny*result->x + xs;                     \
                            if (result->type==CB_VAL) {                 \
                                for (_x=xs; _x<xe; _x++, p++) TESTBG1(wtcol, wt); \
                            } else {                                    \
                                p4 = p * result->type;                  \
                                for (_x=xs; _x<xe; _x++, p++, p4+=result->type) TESTBG4(wtcol, wt); \
                            }                                           \
                        }                                               \
                    }
                    i = ceil(ct_crad);
...
#undef AAPIX
                    ...
                    ye = MIN2((int)ceil(bkh_b[3]*ct_crad + y), result->y - 1);
                    for (sy=ys; sy<=ye; sy++) {
                        ...
                        if (found) {
                            ...
                            if (result->type==CB_VAL) {
                                ...
                                p4 = p * result->type;
                                for (sx=xs; sx<xe; sx++, p++, p4+=result->type) TESTBG4(wtcol, wt);
                            }
                        }
                    }
...
#undef TESTBG1
                ...
                for (s=0; s<maxsam; ++s) {
                    ...
                    if ((sx<0) || (sx >= result->x) || (sy<0) || (sy >= result->y)) continue;
                    p = sx + sy*result->x;
                    p4 = p * result->type;
                    if (nqd->bktype==0) {    // Disk
                    ...
                    if (ct_crad > nqd->bthresh) {  // if center blur > threshold
                        if (crad->rect[p] > nqd->bthresh) { // if overlap pixel in focus, do nothing, else add color/weigbt
                            result->rect[p4] += ctcol[0] * lwt;
                            if (result->type != CB_VAL) {
                                result->rect[p4+1] += ctcol[1] * lwt;
                                result->rect[p4+2] += ctcol[2] * lwt;
                                result->rect[p4+3] += ctcol[3] * lwt;
                            }
                            wts->rect[p] += lwt;
                        }
                    }  else {
    ...
    for (y=0; y<result->y; y++) {
        unsigned int p = y * result->x;
        unsigned int p4 = p * result->type;
        int x;
